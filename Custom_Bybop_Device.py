...
class BebopDrone(Device):
	...
	#HERE ARE WHERE TO CREATE CUSTOM COMMANDS
	def take_off(self):
	...
	#THE FOLLOWING COMMANDS WERE CUSTOM MADE
	#USING THE FOLLOWING INFORMATION, MORE CUSTOM COMMANDS CAN BE MADE TO BE SENT TO THE BEBOP DRONE
	#each integer value represents the following: flag,pitch,roll,yaw,and gaz.flag at 0 locks pitch and roll.
	def rise(self):
		self.send_data('ardrone.Piloting.PCMD',0,0,0,0,10,0)
	def lower(self):
		self.send_data('ardrone.Piloting.PCMD',0,0,0,0,-10,0)
	def forward(self):
		self.send_data('ardrone.Piloting.PCMD',1,0,10,0,0,0)
	def back(self):
		self.send_data('ardrone.Piloting.PCMD',1,0,-10,0,0,0)
	def left(self):
		self.send_data('ardrone.Piloting.PCMD',1,-10,0,0,0,0)
	def right(self):
		self.send_data('ardrone.Piloting.PCMD',1,10,0,0,0,0)
	def rright(self):
		self.send_data('ardrone.Piloting.PCMD',0,0,0,-10,0,0)
	def rleft(self):
		self.send_data('ardrone.Piloting.PCMD',0,0,0,10,0,0)
